...
**Repo:** kynotes-server
**PR:** #6 — https://github.com/Busness-app/kynotes-server/pull/6 (merged prerequisite; no implementation PR for this plan)
**Worktree:** /home/yoshi/busness.app/kynotes-server (branch master)

Prepared 2026-09-05 for Myslop #290. Owner: `copperfinch`, claimed in #297 in
`kynotes-kyrecovery-deposit`. Scope of this session: claim and planning only.
...
Production code has no `internal/backup` package or recoveryclient integration.
...
- [ ] Recheck Myslop ownership and new posts, fetch origin, and inspect worktree/status.
...
- [ ] Read current owning docs and the shared-library handoff's reviewer findings.
...
- [ ] Establish the existing test baseline and record the source SHA. Keep existing
...
- [ ] Replace `ParseClaims`' unverified JWS decoding with a reusable
...
- [ ] Require a verified ID token before account/session mutation. If userinfo remains
...
- [ ] Trace subject lookup, username adoption, provisioning, and role assignment.
...
- [ ] Replace body-only HMAC with `syncauth.Middleware` on every webhook alias. Load
...
- [ ] Wire one shared replay guard across aliases and requests. Prove duplicate events
...
Start from merged authentication repair. Reconcile Plan B to v0.5.1; remove obsolete
...
- [ ] Add `internal/backup` settings/sealer glue, collector, shared service, drill
...
- [ ] Seal a live SQLite snapshot, all effective deployment secrets (including values
...
- [ ] Inventory every blob referenced by the snapshot. Important correction from code:
...
- [ ] Bind the token sealer to the existing deployment secret with a stable,
...
- [ ] Implement pin, pair, unpair, schedule, status, export, run and drill operations.
...
- [ ] Wire `internal/httpapi/backup_routes.go`, CLI, and `internal/app/serve.go`.
...
- [ ] Add `backup-drill`, `export-capsule`, `deposit`, and sealed `restore` commands.
...
- [ ] Build the admin backup screen from existing UI patterns: key, remote, local and
...
- [ ] Implement v0.5.1 drill callback
...
- [ ] Add `github.com/Busness-app/ky-primitives/offsite@v0.1.0` (repository tag
...
- [ ] Add the next unused migration for replica inventory, with digest foreign key and
...
- [ ] Implement `internal/mirror` sync/fetch over every required blob digest. Reuse
...
- [ ] Record a replica only after successful transfer; for SMB `ErrObjectExists`, read
...
- [ ] Wire mirror/fetch/test-target CLI, scheduled/manual runs, mirror-only admin route,
...
- [ ] Document config/env/compose settings, SFTP pin verification, library SMB limits,
...
Claim and plan are complete; implementation and runtime tests have not started. Next
action is Phase 0, then the focused authentication PR. The folder remains claimed by
`copperfinch`. This complete document is mirrored to Myslop before ending the session.
DOX pass: this planning document changes no shipped behavior or frozen contract;
existing AGENTS.md files are intentionally unchanged. Reconcile owning docs with each
implementation PR rather than describe planned behavior as already shipped.
